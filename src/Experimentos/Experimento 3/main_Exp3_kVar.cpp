...
int main (int argc, char **argv){
	...
	if(modo == 1){
		...
		std::vector<int> vecinos = {1, 5, 10, 20, 30, 40, 50, 100};
		std::vector<double> promHR(vecinos.size());

		for(int v = 0 ; v < vecinos.size() ; ++v){
				
		cout<< "---------------------------- k = " << vecinos[v] << " -----------------------------------"<< endl;		
	 	...
	 	for(int i = 0; i < K; i++){
			...
			vector<int> resultados;
	   		...
	   		for(int j = 0; j < testing.size(); j++){
		        ...
		        vector<unsigned char>& imagen = testing[j];  
				resultados.push_back(KnnD(labels2, estructura.mTransformada(), estructura.transformarDigito(imagen), vecinos[v]));
		       
	        }
	        
	        promHR[v]+= hitRate(resultados, indicesDeTest, labels); //acumulo el hitRate de cada pasada para un k fijo
		...
		promHR[v] = (double)(promHR[v]/K);  //deja el promedio en la posición v para el k vecinos[v]	
		cout<< "PROMEDIO HR DE k =  "<< vecinos[v] << ": "<<  promHR[v] << endl; 
				
		}
		
		unsigned int kOptimo = vecinos[maxPos(promHR)];   //devuelvo el k tal que el hitrate promedio es máximO
		cout<< "k OPTIMO DEL EXPERIMENTO(con peso): "<< kOptimo << endl;
	...
	if(modo == 2){
		...
		for(int i = 0; i < K; i++){
			...
			cout << "PLS-DA + k-NN. Trabajando... iteracion: "<< (i+1) << " de "<< particiones.size() <<endl;
			...
			int n = testing.size();
	   		...
	   		for(int j = 0; j < testing.size(); j++){
		        ...
		        vector<unsigned char>& imagen = testing[j];  	
				resultados.push_back(KnnD(labels2, dbTransformada, plsTransformarDigito(imagen, cambioDeBase, Media, n), vecinos[v]));
