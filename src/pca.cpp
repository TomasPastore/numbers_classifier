...
void PCA::prodTraspuesta(const vector < vector < double > >& X, vector < vector < double > >& M) {
	...
	//COUT("Inicia producto traspuesta")
	...
	for(int i = 0; i < M.size(); i++) {
		...
		for(int j = 0; j < M[i].size(); ++j ) {
			double valor = 0;
			for( int k = 0; k < X.size(); ++k) {  // optimización k < (500 > X.size() ? X.size(): 500) ver experimento 5 para la justificación de ese 500 en vez de n.
				valor+= X[k][i]*X[k][j];
			}
			M[i][j]=valor;
		}
